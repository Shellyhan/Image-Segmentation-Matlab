...
% 
% Comments:
%     Much better result than K-means, number of clusters are not limited,
%     so preserves the details of the image.
%     However, it's much slower compared to K-means.
%     Eventhough it gives a much better result, some parts are still
%     'blurred out'.
% 
...
I = double( imread('fruit.jpg') );
bwS = 20;       %Bandwidth for spacial
bwC = 32;       %Bandwidth for color
...
while ~conv
    ...
    for i = -bwS:bwS
        for j = -bwS:bwS
            if ( i~=0 || j~=0 )
                % not considering the spatial feature, thus set to 1:
                Spatial = 1;
                CurrX =  Array(a+i:2*a+i-1, b+j:2*b+j-1, 1:c);
                % calculated the square difference:
                DiffX = (result-CurrX).^2;
                % Color feature:
                CMeans = repmat(prod(reshape(wTable(DiffX+1), a, b, c), 3), [1, 1, c]);
                % taking weights from just one feature array:
                newFeature = Spatial.*CMeans;
